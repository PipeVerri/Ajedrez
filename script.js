...
function terminarJugada(e, tthis) {
    if (sourcePiece != "" && clickAgarrar == true) {
        if (!$(tthis).hasClass("casillaPosible")) {
            //return false;
        }
        soltarPieza();
        $(tthis).html("");
...
function movimientoPeon(tipo, color) {
    ...
    //esto es para no marcar las 2 celdas de los costados del peon
    if (iMove == false) {
        if (columna == 8) {
            if (color == "white") {
                var celda = "#celda-" + (fila - 2) + "-" + (columna - 1);
            } else {
                var celda = "#celda-" + (fila + 2) + "-" + (columna - 1);
            }
            $(celda).removeClass("casillaPosible");
        } else if (columna == 1) {
            if (color == "white") {
                var celda = "#celda-" + (fila - 2) + "-" + (columna + 1);
            } else {
            ...
            if (color == "white") {
                var celda = "#celda-" + (fila - 2) + "-" + (columna + 1);
                var celda2 = "#celda-" + (fila - 2) + "-" + (columna - 1);
            } else {
                var celda = "#celda-" + (fila + 2) + "-" + (columna + 1);
                var celda2 = "#celda-" + (fila + 2) + "-" + (columna - 1);
            }
            $(celda).removeClass("casillaPosible");
            $(celda2).removeClass("casillaPosible");
        }
    }

    //esto es saber si puedo comer piezas
    if (columna == 8) {
        if (color == "white") {
            var celda = "#celda-" + (fila - 1) + "-" + (columna - 1);
        } else {
            var celda = "#celda-" + (fila + 1) + "-" + (columna - 1);
        }
        if ($(celda).find(".pieza").length > 0) {
            if ($(celda).find(".pieza").hasClass(color)) {
                $(celda).removeClass("casillaPosible");
            }
        } else {
            $(celda).removeClass("casillaPosible");
        ...
        if (color == "white") {
            var celda = "#celda-" + (fila - 1) + "-" + (columna + 1);
            var celda2 = "#celda-" + (fila - 1) + "-" + (columna - 1);
        } else {
            var celda = "#celda-" + (fila + 1) + "-" + (columna + 1);
            var celda2 = "#celda-" + (fila + 1) + "-" + (columna - 1);
        }
        ...
        if ($(celda2).find(".pieza").length > 0) {
            if ($(celda2).find(".pieza").hasClass(color)) {
                $(celda2).removeClass("casillaPosible");
            }
        } else {
            $(celda2).removeClass("casillaPosible");
    ...
    //si tiene una pieza adelane no puede avanzar
    if (color == "white") {
        var celda = "#celda-" + (fila - 1) + "-" + columna;
    } else {
        var celda = "#celda-" + (fila + 1) + "-" + columna;
    }
    if ($(celda).find(".pieza").length > 0) {
...
function movimientoAlfil(tipo, color) {
    var cordenadasPosibles = [];
    var idArray = idPieza.split("-");
    var fila = parseInt(idArray[1]);
    var columna = parseInt(idArray[2]);
    ...
    for (var a of cordenadasPosibles) {
        $(a).addClass("casillaPosible");
    }
}